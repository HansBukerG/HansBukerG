...
public class BusquedaAnchuraMultiObjetivoInformadaConLadron extends TimerTask implements Constantes{
    ...
    public BusquedaAnchuraMultiObjetivoInformadaConLadron(Mapa mapa,Cartero cartero) {
    ...
    private void moverArriba(Estado e) {  
        if ( e.y > 0 ) {
            if ( cartero1.nohayPared(e.x, e.y-1)) { 
                 ...
                 ladron = new Estado(mapa.lienzoPadre.ladron.ladron.x,mapa.lienzoPadre.ladron.ladron.y,'N',e);
                 arriba.prioridad = (int) (distancia(arriba,objetivo)+distancia2(arriba,ladron));
    ...
    private void moverAbajo(Estado e) {
        ...
        if ( e.y+1< alturaMundoVirtual ) { 
            if ( cartero1.nohayPared(e.x, e.y+1) ) {
                 ...
                 Estado ladron;
                    ladron = new Estado(mapa.lienzoPadre.ladron.ladron.x,mapa.lienzoPadre.ladron.ladron.y,'N',e);
                    abajo.prioridad = (int) (distancia(abajo,objetivo)+distancia2(abajo,ladron));
    ...
    private void moverIzquierda(Estado e) {
        if ( e.x > 0 ) {
            if ( cartero1.nohayPared(e.x-1, e.y) ) {
                ...
                Estado ladron;
                    ladron = new Estado(mapa.lienzoPadre.ladron.ladron.x,mapa.lienzoPadre.ladron.ladron.y,'N',e);
                izquierda.prioridad = (int) (distancia(izquierda,objetivo)+distancia2(izquierda,ladron));
    ...
    private void moverDerecha(Estado e) {
        ...
        if ( e.x < anchuraMundoVirtual-1 ) {
            if ( cartero1.nohayPared(e.x+1, e.y) ) {
                ...
                Estado ladron;
                    ladron = new Estado(mapa.lienzoPadre.ladron.ladron.x,mapa.lienzoPadre.ladron.ladron.y,'N',e);
                derecha.prioridad = (int) (distancia(derecha,objetivo)+distancia2(derecha,ladron));
    ...
    public double distancia2(Estado primero, Estado segundo) 
    {
        if(HEURISTICALADRON==true)
        {
            double valor;
            double parte1=Math.pow(Math.abs(primero.x-segundo.x),2);
            double parte2=Math.pow(Math.abs(primero.y-segundo.y),2);

--PB_Registro_detallado
/**INICIO COMENTARIO 

Ignorar este documento, sirve para poder generar sp de consultas de reportes

FIN COMENTARIO**/



Declare @ofre_codigo		integer
Declare @cco_codigo			decimal
Declare @rep_codigo			decimal
Declare @edu_codigo			integer
Declare @mae_rut			integer
declare @rut_cliente		integer
Declare @fecha_hasta		datetime
Declare @Fecha_desde		datetime
Declare @serv_codigo		integer
Declare @cate_codigo		integer
Declare @saa_codigo			integer
Declare @tpr_correlativo	integer
Declare @estad_codigo		integer
Declare @hora_desde			varchar(8)
Declare @hora_hasta			varchar(8)

select @estad_codigo	= 3

select @ofre_codigo		= 20
select @cco_codigo		= 113
select @rep_codigo		= 30302000
select @edu_codigo		= 7

--CREATE proc dbo.pb_atenciones_por_servicio
--(
--	 	@ofre_codigo		integer
--	, 	@cco_codigo			decimal
--	, 	@rep_codigo			decimal
...
(
	...
	nombre VARCHAR(60),
	estad_codigo INT,
	tpr_descripcion VARCHAR(20),
	tpr_correlativo INT,
	saa_descripcion VARCHAR(20),
	saa_codigo INT,
...
join documentos_ubb.dbo.motivo_atencion as motivo with(nolock) on  motivo.FILA_FECHA    = filas.FILA_FECHA
and   motivo.FILA_CODIGO   = filas.FILA_CODIGO
and   motivo.OFRE_CODIGO   = filas.OFRE_CODIGO
and   motivo.CCO_CODIGO    = filas.CCO_CODIGO
...
on  motivo.MOAT_FECHA    = atencion.MOAT_FECHA  
and motivo.MOAT_CODIGO   = atencion.MOAT_CODIGO
and motivo.FILA_CODIGO   = atencion.FILA_CODIGO
and motivo.FILA_FECHA    = atencion.FILA_FECHA
and motivo.OFRE_CODIGO   = atencion.OFRE_CODIGO
and motivo.CCO_CODIGO    = atencion.CCO_CODIGO
and motivo.REP_CODIGO    = atencion.REP_CODIGO
and motivo.EDU_CODIGO    = atencion.EDU_CODIGO
...
and atencion.CCO_CODIGO     = servicios.CCO_CODIGO
...
and  atencion.CCO_CODIGO   = asignacion.CCO_CODIGO
...
and filas.CCO_CODIGO	= @cco_codigo
...
on  motivo.FILA_FECHA    = filas.FILA_FECHA
and   motivo.FILA_CODIGO   = filas.FILA_CODIGO
and   motivo.OFRE_CODIGO   = filas.OFRE_CODIGO
and   motivo.CCO_CODIGO    = filas.CCO_CODIGO
and   motivo.REP_CODIGO    = filas.REP_CODIGO
...
SELECT
	cate_nombre
,	serv_nombre	
,	CONVERT(VARCHAR(10),cast(AVG(cast(fin_atencion-inicio_atencion as float)) as datetime),108) as duracion_atencion
FROM @temp

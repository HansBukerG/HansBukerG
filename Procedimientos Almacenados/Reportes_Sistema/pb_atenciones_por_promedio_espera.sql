...
/**INICIO_DATOS_PROCEDIMIENTO	
	...
	@-Parametro_Salida2: CONVERT(VARCHAR(10),cast(AVG(cast(inicio_atencion-hora_fecha_registro as float)) as datetime),108) as promedio_espera

	@-Descripcion_Objeto0: SP de reporte para promedios de espera del Sistema de Gestión de Filas
	@-Regla_Objeto0: Debe existir una oficina de reparticion con actividad para poder generar reporte
	@-String_Prueba0: EXEC Documentos_ubb.dbo.pb_atenciones_por_promedio_espera;
	
FIN_DATOS_PROCEDIMIENTO**/	


alter proc dbo.pb_atenciones_por_promedio_espera
(
	 	@ofre_codigo		integer
	, 	@cco_codigo			decimal
	, 	@rep_codigo			decimal
...
SELECT
	cate_nombre
,	serv_nombre	
,	CONVERT(VARCHAR(10),cast(AVG(cast(inicio_atencion-hora_fecha_registro as float)) as datetime),108) as promedio_espera
FROM @temp

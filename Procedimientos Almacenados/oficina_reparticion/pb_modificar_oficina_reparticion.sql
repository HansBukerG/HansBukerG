/**INICIO_DATOS_PROCEDIMIENTO 
   ...
   @-Descripcion_Objeto0: Modifica los parametros de una oficina de reparticion
   @-Regla_Objeto0: Debe existir una oficina de reparticion previamente para poder ser modificada.
   @-String_Prueba0: EXEC Documentos_ubb.dbo.pb_modificar_oficina_reparticion;
   
FIN_DATOS_PROCEDIMIENTO**/ 


alter proc dbo.pb_modificar_oficina_reparticion
(  
      @OFRE_CODIGO          integer
   ,  @CCO_CODIGO           decimal
   ,  @REP_CODIGO           decimal
...
BEGIN TRANSACTION;
   ...
   UPDATE DOCUMENTOS_UBB.dbo.OFICINA_REPARTICION with(updlock)
   SET   OFRE_NOMBRE_OFICINA = @OFRE_NOMBRE_OFICINA
   ,     OFRE_ABREVIACION    = @OFRE_ABREVIACION
   ,     OFRE_FECHA_INICIO   = @OFRE_FECHA_INICIO
   ,     OFRE_FECHA_TERMINO  = @OFRE_FECHA_TERMINO
   ,     OFRE_FECHA_REGISTRO = @OFRE_FECHA_REGISTRO
   ,     OFRE_LOG_REGISTRO   = @OFRE_LOG_REGISTRO 
